...
const BACKEND = process.env.API_INTERNAL_URL || process.env.NEXT_PUBLIC_API_URL?.replace(/\/api\/?$/, "") || "http://localhost:9000";
...
/**
 * SSE proxy to the zingbee-ultra backend /api/chat/stream.
    ...
    const nodeReq = transport.request(
      ...
      (nodeRes) => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            nodeRes.on("data", (chunk: Buffer) => { controller.enqueue(chunk); });
            nodeRes.on("end", () => { controller.close(); });
            nodeRes.on("error", (err) => { controller.error(err); });
          },
          cancel() { nodeRes.destroy(); },
        });
