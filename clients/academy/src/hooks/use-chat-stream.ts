...
import type { ChatMessage, ExecStep } from "@/lib/chat-types";

// Keep in sync with clients/red-team/hooks/use-chat-stream.ts VocabTerm.
// (A "used" status — productive-vocabulary tracking — is planned in ADO #65;
// add it here when the server emits it, not before.)
...
interface KnowledgeStats {
  ...
  facts_taught_texts?: string[];
  facts_assessed_texts?: string[];
  facts_mastered_texts?: string[];
  all_facts?: string[];
  current_fact?: string;
  current_step?: string;
...
interface UseChatStreamReturn extends ChatStreamState {
  ...
  /** Register a callback fired on each raw SSE token (for voice TTS feeding) */
  setOnToken: (cb: ((token: string) => void) | null) => void;
  /** Register a callback fired when the SSE stream completes */
...
export function useChatStream(): UseChatStreamReturn {
  ...
  const [taughtTexts, setTaughtTexts] = useState<string[]>([]);
  const [assessedTexts, setAssessedTexts] = useState<string[]>([]);
  const [masteredTexts, setMasteredTexts] = useState<string[]>([]);
  ...
  // Voice mode callbacks — fired on SSE token/done events
  ...
  const applyKnowledge = useCallback((knowledge: KnowledgeStats | null) => {
    ...
    if (knowledge.all_facts?.length) setAllFacts(knowledge.all_facts);
    if (knowledge.current_fact !== undefined) setCurrentFact(knowledge.current_fact || "");
    if (knowledge.facts_taught_texts) setTaughtTexts(knowledge.facts_taught_texts);
    if (knowledge.facts_assessed_texts) setAssessedTexts(knowledge.facts_assessed_texts);
    if (knowledge.facts_mastered_texts) setMasteredTexts(knowledge.facts_mastered_texts);
    if (knowledge.capsule_name) setCapsuleName(knowledge.capsule_name);
    ...
    if (knowledge.age_range) setAgeRange(knowledge.age_range);
  }, []);
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (payload.type === "thinking") {
            ...
            // Notify voice mode hook (if active) with raw token
            onTokenRef.current?.(payload.content);
            ...
            // The server-side session already ended (AFK sweep, another tab,
            // stale page). Don't render a reply — surface a session-end so the
            // page can inform the student and route to the summary screen.
            clearStreamingTimeout();
            stopThinking();
            setStreaming(false);
            setEndSessionData({ should_end: true, session_expired: true });
            ...
            if (payload.image_url) {
              ...
              // No image coming (EVIDENCE) — remove the loading placeholder
              setMessages((prev) => prev.filter((m) => !m.imageLoading));
            }
            ...
            stopThinking();
            setStreaming(false);
            // Image event arrives after done — keep UI locked briefly.
            // Safety: auto-unlock after 5s if no image event comes (EVIDENCE/errors).
            setWaitingForImage(true);
            setTimeout(() => setWaitingForImage(false), 5000);
            ...
            // Notify voice mode hook that stream is complete
            onDoneRef.current?.();
          }
        }
        if (done) break;
  ...
  const setOnToken = useCallback((cb: ((token: string) => void) | null) => { onTokenRef.current = cb; }, []);
  const setOnDone = useCallback((cb: (() => void) | null) => { onDoneRef.current = cb; }, []);

  return {
    messages, execLog, streaming, waitingForImage, thinkingLabel, factsState, totalFacts, allFacts, currentFact, taughtTexts, assessedTexts, masteredTexts, capsuleName,
    tokenCount, questionsAsked, correctAnswers, suggestions, currentPhase,
    currentStep, sessionActive, endSessionData,
    vocabBank, definitionsHidden, ageRange,
