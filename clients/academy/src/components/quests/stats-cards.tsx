...
export function StatsCards() {
  ...
  // Mock lessons with translation keys - in production these would come from the API
  ...
  useEffect(() => {
    // Load user's language preference for date formatting
    const lang = localStorage.getItem("preferredLanguage") || "en"
    setUserLocale(LOCALE_MAP[lang] || "en-US")

    const loadStats = async () => {
      try {
        const authData = await apiClient.auth.me()
        if (!authData.student?.id) return
        const statsResp = await apiClient.get<{
          topics_covered: number
          day_streak: number
          minutes_learning: number
        }>(`/chat/stats?student_id=${authData.student.id}`)
        setStats({
          lessonsCompleted: statsResp.topics_covered,
          currentStreak: statsResp.day_streak,
          totalTime: statsResp.minutes_learning,
        })
      } catch (e) {
    ...
    <>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t("stats.topicsCovered")}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Dialog open={openDialog === "lessons"} onOpenChange={() => setOpenDialog(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("stats.topicsCovered")}</DialogTitle>
            <DialogDescription>
              {t("stats.topicsDescription", { count: completedLessons.length })}
            </DialogDescription>
          </DialogHeader>
            ...
            {completedLessons.map((lesson) => (
              ...
              >
                ...
                <div className="text-right">
                  <Badge variant="secondary">{new Date(lesson.completedAt).toLocaleDateString()}</Badge>
                </div>
              </div>
            ))}
          </div>
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-semibold">
                    {new Date(day.date).toLocaleDateString(userLocale, {
                      weekday: "long",
                      month: "short",
                      day: "numeric",
                    })}
